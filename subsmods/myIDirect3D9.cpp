...
RealDirect3DCreate9Type *RealDirect3DCreate9 = 0;


// Exported function (faking d3d9.dll's one-and-only export)
IDirect3D9* WINAPI Direct3DCreate9(UINT SDKVersion) {
	if (!RealDirect3DCreate9) {
		HMODULE h = LoadLibraryW(L"d3d9_alt.dll");
		if (h) {
			RealDirect3DCreate9 = (RealDirect3DCreate9Type*)GetProcAddress(h, "Direct3DCreate9");
		}
		if (!RealDirect3DCreate9) {
			wchar_t path[MAX_PATH*3];
			int l = GetEnvironmentVariableW(L"SYSTEMROOT", path, sizeof(path)/2);
			if (l > 3 && l < MAX_PATH*2) {
				wcscpy(path+l, L"\\System32\\d3d9.dll");
			}
			else return 0;
			HMODULE h = LoadLibraryW(path);
			RealDirect3DCreate9 = (RealDirect3DCreate9Type*)GetProcAddress(h, "Direct3DCreate9");
			if (!RealDirect3DCreate9) return 0;
		}
	}
	IDirect3D9 *out = RealDirect3DCreate9(SDKVersion);
	if (!out || state.Excluded) {
		return out;
	}

	return new myIDirect3D9(out);
}
...
ULONG    __stdcall myIDirect3D9::Release(void)
{
	...
	// so do we here
	if (count == 0) {
		//MessageBox(0, "IDirect3D9 Object Destroyed", "IDirect3D9", MB_OK);
  	    delete(this); 
	}
...
HRESULT __stdcall myIDirect3D9::CreateDevice(UINT Adapter,D3DDEVTYPE DeviceType,HWND hFocusWindow,DWORD BehaviorFlags,D3DPRESENT_PARAMETERS* pPresentationParameters,IDirect3DDevice9** ppReturnedDeviceInterface)
{
	if (config.width && config.height) {
		pPresentationParameters->BackBufferWidth = config.width;
		pPresentationParameters->BackBufferHeight = config.height;
	}
	...
	*ppReturnedDeviceInterface = new myIDirect3DDevice9(*ppReturnedDeviceInterface, pPresentationParameters->BackBufferWidth, pPresentationParameters->BackBufferHeight);
