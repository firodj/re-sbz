/* Contains code for wrapper around standard Direct3DDevice9 structure
 * (And Direct3DCreate9 and IDirect3D9->CreateDevice).
 * Only interesting functions here are the first couple,
 * which have to deal with setting up the Direct3DDevice9 wrapper.
 *
 * myIDirect3DDevice9::EndScene(), which does all the work, and
 * myIDirect3DDevice9::Release() are both in D3D9TextOverlay.cpp.
 */

// Needed to display version text once a D3D window is created.
...
#include <stdlib.h>
/*
HRESULT WINAPI MyDirect3D9CreateDevice(
	IDirect3D9 *d3ddev,
	UINT Adapter,
	...
	HRESULT res = RealDirect3D9CreateDevice(
	  d3ddev,
	  Adapter,
	  DeviceType,
	  hFocusWindow,
	...
	if (!FAILED(res)) {
		...
		if (justStarted) {
			CleanupAllLines();
			AddText(0, "0 20000 " VERSION, D3DCOLOR_RGBA(255,0,0,255));
			AddText(0, "0 20000      Ctrl-D toggles debug mode", D3DCOLOR_RGBA(0,255,255,255));
			AddText(0, "0 20000      Ctrl-L toggles logging to file", D3DCOLOR_RGBA(0,255,255,255));
			AddText(0, "0 20000      Ctrl-B logs last 15 lines", D3DCOLOR_RGBA(0,255,255,255));
			AddText(0, "0 20000      Ctrl-R reloads config file", D3DCOLOR_RGBA(0,255,255,255));
			AddText(0, "0 20000      Ctrl-A toggles showing all playing files (Debug mode/log only)", D3DCOLOR_RGBA(0,255,255,255));
			AddText(0, "0 20000          Files that tend to contain only effects are otherwise skipped", D3DCOLOR_RGBA(0,255,255,255));
			justStarted = 0;
		}
		* ppReturnedDeviceInterface = new myIDirect3DDevice9(* ppReturnedDeviceInterface);
		// Alternative to the whole myDirect3DDevice9 thing.
		// Would be better solution, if it worked, but a lot of commands modify the vtable,
		// oddly enough.

		//d = * ppReturnedDeviceInterface;
		/*DWORD* lpVtabl = *(DWORD**)*ppReturnedDeviceInterface;
		MEMORY_BASIC_INFORMATION mbi;

		if(VirtualQuery((LPVOID)(lpVtabl), &mbi, sizeof(mbi)) == sizeof(mbi)) {
			DWORD dwOldProtect;
			if (VirtualProtect(mbi.BaseAddress, ((DWORD)lpVtabl + 4*0x2B)-(DWORD)mbi.BaseAddress, PAGE_EXECUTE_READWRITE, &dwOldProtect)) {
				if (!RealDirect3D9DeviceEndScene) RealDirect3D9DeviceEndScene = (RealDirect3D9DeviceEndSceneType*) lpVtabl[0x2A];
				lpVtabl[0x2A] = (DWORD) fun4;
				VirtualProtect(mbi.BaseAddress, ((DWORD)lpVtabl + 4*0x2B)-(DWORD)mbi.BaseAddress, dwOldProtect, &dwOldProtect);
			}
		}
		//*/
/*	}
...
myIDirect3DDevice9::myIDirect3DDevice9(IDirect3DDevice9* pOriginal, int w, int h) {
	width = w;
	height = h;
	//MessageBox(0, "IDirect3DDevice9 Object Created", "IDirect3DDevice9", MB_OK);
	refCount = 1;
