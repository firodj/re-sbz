...
typedef IDirect3D9* (WINAPI RealDirect3DCreate9Type)(UINT version);
typedef HRESULT (WINAPI RealDirect3D9CreateDeviceType)(
	IDirect3D9 *d3ddev,
	UINT Adapter,
...
extern RealDirect3DCreate9Type* RealDirect3DCreate9;
//extern RealDirect3D9CreateDeviceType* RealDirect3D9CreateDevice;

class myIDirect3DDevice9 : public IDirect3DDevice9
{
	int refCount;
public:
	// Back buffer size, declared on creation.
	int width, height;

    myIDirect3DDevice9(IDirect3DDevice9* pOriginal, int w, int h);
...
IDirect3D9 * WINAPI MyDirect3DCreate9(UINT version);
