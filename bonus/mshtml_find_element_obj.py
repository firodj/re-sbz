				...
				for nref in XrefsTo(cref,0):
					...
					if not coderef in address_list:
						...
						if not funcname == None:
							...
							foundHeap = False
							while readback < 40:
								instr = idc.generate_disasm_line(coderef-readback, 0)
								#if debug:
								#	print(instr)
								disable_mask = idc.get_inf_attr(idc.INF_SHORT_DN) 
								thefuncname = idc.demangle_name(funcname, disable_mask)	
								# x86 block				
...
def main():
	...
	if (nr_of_imports > 0):
		for i in range(0,nr_of_imports):
...
# URL: https://github.com/uf0o/idapython-scripts/blob/master/scripts/find_element_obj.py
