...
def rename_functions():
    patterns = [
        {
            ...
            "hex": "558bec51894dfc8b45fc8b008be55dc3", 
            "description": "Returns [this+0]"
        },
        {
            "name": "Empty_Member_Func",
            "hex": "558bec51894dfc8be55dc20400",
            "description": "Empty member function with retn 4"
        }
    ]
    ...
    for pattern in patterns:
        target_bytes = bytes.fromhex(pattern["hex"])
        prefix = target_bytes[:8] # Optimization: check prefix first
        pattern_len = len(target_bytes)
        pattern_count = 0
        ...
        for func_ea in idautils.Functions():
            # Quick check first
            if ida_bytes.get_bytes(func_ea, 8) != prefix:
                continue

            # Full check
            func_bytes = ida_bytes.get_bytes(func_ea, pattern_len)
            
            if func_bytes == target_bytes:
